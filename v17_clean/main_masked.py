...
import time
import random
from evaluate_masked import evaluate_model_by_type as evaluation_function
from datasets import RPMFullSentencesRaw_v4 as rpm_dataset
...
def main_BERT(VERSION, RESULTS_FOLDER):
    ...
    train_df, val_df, test_df = gather_files_by_type(root_dir)

    ''' Transformer model v9 '''
    train_dataset = rpm_dataset(train_df, device=device)
    val_dataset = rpm_dataset(val_df, device=device)
