...
def evaluate_model(model, dataloader, autoencoder, save_path, device):
    ...
    with torch.no_grad():
        ...
        for idx, (inputs, targets, imagetensors, target_nums, embeddings) in enumerate(dataloader):
            ...
            min_indices = pick_answers(outputs, candidates).cpu()
...
def main():
    ...
    transformer_model = TransformerModel(depth=8).to(device)
    if num_gpus > 1:
    ...
    # Evaluate the model
    proportion_correct = evaluate_model(transformer_model, val_dataloader, autoencoder, save_path='../tr_results/v1/', device=device)
    print(f"Proportion of answers correct: {proportion_correct}")

    output_file_path = "../tr_results/v1/proportion_correct.txt"
