...
class TransformerModelv26(nn.Module): # takes in images, embeds, performs self-attention, and decodes to image
    def __init__(self,
                 embed_dim=512,
                 symbol_factor=1,
                 grid_size=3,
        ...
        super(TransformerModelv26, self).__init__()
        ...
        self.symbols_2 = nn.Parameter(normal_initializer(torch.empty(12, self.model_dim * self.symbol_factor)))
        ...
        self.mu_mlp = nn.Sequential(
            nn.Linear(2 * self.model_dim, ternary_mlp_ratio * self.model_dim),
            nn.ReLU(),
            nn.Linear(ternary_mlp_ratio * self.model_dim, self.model_dim),
            nn.Dropout(p=ternary_drop)
        )
    ...
    def ternary_mlp(self, x):
        """
        Perform the ternary operation Φ_MLP(x_1, x_2, x_3) for rows and columns across the sequence.
        Performs higher-order binary operation mu_MLP(y_1, y_2) for pairs of rows and cols
        Input x is of shape (batch_size, 9, embed_dim)
        Output is of shape (batch_size, 12, embed_dim)
        """

        batch_size = x.size(0)

        slice_idx_1 = torch.tensor([0, 3, 6, 0, 1, 2])
        increment = torch.tensor([1, 1, 1, 3, 3, 3])

        # Extract x1, x2, x3 for all sliding windows
        x_1 = x[:, slice_idx_1, :]  # Shape: (batch_size, 6, embed_dim)
        x_2 = x[:, slice_idx_1 + increment, :]  # Shape: (batch_size, 6, embed_dim)
        x_3 = x[:, slice_idx_1 + 2 * increment, :]  # Shape: (batch_size, 6, embed_dim)

        x_1 = x_1.reshape(batch_size * 6, -1)
        x_2 = x_2.reshape(batch_size * 6, -1)
        x_3 = x_3.reshape(batch_size * 6, -1)
        ...
        rows_cols = rules.reshape(batch_size, 6, -1)

        slice_idx_2 = torch.tensor([0, 0, 1, 3, 3, 4])
        slice_idx_3 = torch.tensor([1, 2, 2, 4, 5, 5])

        y_1 = rows_cols[:, slice_idx_2, :].reshape(batch_size * 6, -1)
        y_2 = rows_cols[:, slice_idx_3, :].reshape(batch_size * 6, -1)
    ...
    def forward(self, sentences, feedback):
        ...
        dist_reshaped = self.guesser_head(z_reshaped)
        ...
        return dist, recreation, embeddings, reas_raw, reas_decoded, z_reshaped, loss_weights, feedback
